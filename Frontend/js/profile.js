...
async function saveProfile(profileData) {
  try {
    const response = await AuthSystem.apiCall('/users/profile', {
      method: 'PUT',
      body: JSON.stringify(profileData)
    });
    
    if (response.success) {
      AuthSystem.showMessage('Profile updated successfully!', 'success');
    } else {
      AuthSystem.showMessage(response.message, 'error');
    }
  } catch (error) {
    AuthSystem.showMessage('Error updating profile', 'error');
  }
}